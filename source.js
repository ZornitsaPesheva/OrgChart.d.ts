...
OrgChart.prototype._draw = function (readFromCache, action, actionParams, callback) {
    ...
    this.server.read(readFromCache, this.width(), this.height(), vb, action, actionParams, function(response){  
        ...
        for (var id in response.visibleNodes) {
            ...
            content += that.ui.secondLinks(response.visibleNodes, bgnode, that.config, response.boundary, newScale);
...
OrgChart.prototype.getScale = function (viewBox) {
    ...
    return OrgChart.getScale(viewBox, this.width(), this.height(), this.config.scaleInitial);
};
...
OrgChart.prototype.center = function (nodeId, options) {
    var parentState;
    var childrenState;
    var rippleId = nodeId;
    var vertical = true;
    ...
    this._draw(false, OrgChart.action.centerNode, { id: nodeId, options: opt})
};

OrgChart.prototype.fit = function () {
    this.config.scaleInitial = OrgChart.match.boundary;
    this._draw(true, OrgChart.action.init);
};
...
OrgChart.prototype.group = function (sourceBGNodeId, targetBGNodeId) {
    var sourceBGNode = this.getBGNode(sourceBGNodeId);
    var targetBGNode = this.getBGNode(targetBGNodeId);
    var that = this;

    var id = targetBGNode.id;

    if (!sourceBGNode.isGroup && !targetBGNode.isGroup && !sourceBGNode.isChildOfGroup && !targetBGNode.isChildOfGroup) {
        var groupName = OrgChart._randomId();
        ...
        this.addNodeTag(sourceBGNode.id, groupName, true);
        this.addNodeTag(targetBGNode.id, groupName, true);
        var pid = this._get(targetBGNode.id).pid;
        this.link(sourceBGNode.id, pid);
    }
    else if (!sourceBGNode.isGroup && !targetBGNode.isGroup && sourceBGNode.isChildOfGroup && !targetBGNode.isChildOfGroup) {
        var groupName = OrgChart._randomId();
        this.addTag(groupName, {
            group: true,
            groupName: "",
        ...
        this._removeGroupIfHasOneNode(sourceBGNode._groupParentNodeId, sourceBGNode.id);
        this.removeNodeTag(sourceBGNode.id, sourceBGNode._groupParentNodeId, true);
        this.addNodeTag(sourceBGNode.id, groupName, true);
        this.addNodeTag(targetBGNode.id, groupName, true);

        var pid = this._get(targetBGNode.id).pid;

        if (sourceBGNode.id != pid) {
            this.link(sourceBGNode.id, pid);
        }
        else {
            var group = this.getBGNode(sourceBGNode._groupParentNodeId);
            var pid = "";
            for (var i = 0; i < group.bgnodes.length; i++) {
                var childBGNode = group.bgnodes[i];
                if (childBGNode.id != sourceBGNode.id) {
                    pid = childBGNode.id;
                    break;
                }
            }
            ...
            this.link(sourceBGNode.id, pid);
            this.link(targetBGNode.id, pid);
        }
    }
    else if (!sourceBGNode.isGroup && !targetBGNode.isGroup && !sourceBGNode.isChildOfGroup && targetBGNode.isChildOfGroup) {
        var groupName = targetBGNode._groupParentNodeId;
        this.addNodeTag(sourceBGNode.id, groupName, true);
        var tagetGroupNode = this.getBGNode(groupName);
        var pid = tagetGroupNode.bgnodes[0].pid;
        this.link(sourceBGNode.id, pid);
    }
    else if (!sourceBGNode.isGroup && !targetBGNode.isGroup && sourceBGNode.isChildOfGroup && targetBGNode.isChildOfGroup) {
        this._removeGroupIfHasOneNode(sourceBGNode._groupParentNodeId, sourceBGNode.id);
        this.removeNodeTag(sourceBGNode.id, sourceBGNode._groupParentNodeId, true);
        this.addNodeTag(sourceBGNode.id, targetBGNode._groupParentNodeId, true);
    }


    this._draw(false, OrgChart.action.update, { id: id }, function () {
        that.ripple(sourceBGNode.id);
    });
};
...
if (typeof(module) != "undefined"){
    module.exports = OrgChart.VERSION;
}


OrgChart.input = function (label, value, placeholder, readOnly, isImg) {
    ...
    uploadBtnElement.addEventListener("click", function () {
        ...
        fileUpload.onchange = function () {            
            ...
            OrgChart.events.publish('imageuploaded', [file, that.parentNode.querySelector("input")]);            
        };
...
OrgChart.editUI.prototype._createDetailsView = function (node, detailsViewElement) {
    ...
    for (var i = 0; i < fields.length; i++) {
        ...
        if (OrgChart._fieldIsImg(this.obj.config, fields[i])) {
            ...
            fieldContentElement.appendChild(OrgChart.input(fields[i], value, null, true));
        }
    }
...
OrgChart.editUI.prototype._createEditView = function (node, editViewElement) {
    ...
    assistantButtonContentElement.innerHTML = '<div label="isAssistant" style="margin-top: 10px;display:inline-block;">Assistant</div><label class="bg-switch"><input type="checkbox" ' + checked + '><span class="bg-slider round"></span></label>';
    ...
    for (var i = 0; i < fields.length; i++) {
        ...
        if (fields[i] != "tags") {
            if (OrgChart._fieldIsImg(this.obj.config, fields[i])) {
                ...
                fieldContentElement.appendChild(OrgChart.input(fields[i], value, null, false, true));                
            }
            else {
                fieldContentElement.appendChild(OrgChart.input(fields[i], value, null, false));
            }
        }  
    }

    if (!node.isGroup && !node.isChildOfGroup && node.childrenIds.length == 0) {
        fieldContentElement.appendChild(assistantButtonContentElement);
    }

    if (!node.isGroup) {
        fieldContentElement.appendChild(addMoreFieldsElement);
    }
...
OrgChart.editUI.prototype.hide = function (showldUpdateTheNode) {
    ...
    if (showldUpdateTheNode) {//updtae node data only if there is 
        var labelElements = this.wrapperElement.querySelectorAll("[label]");
        for (var i = 0; i < labelElements.length; i++) {
            var label = labelElements[i].getAttribute("label");
            
            if (label != null) {
                var value = labelElements[i].parentNode.getElementsByTagName("input")[0].value;
                if (label === OrgChart.TAGS) {
                    data.tags = value.split(",");
                }
                else if (label === "isAssistant") {
                    var checked = labelElements[i].parentNode.getElementsByTagName("input")[0].checked;
                ...
                else if (data[label] != undefined) {//the field exist in the node data
                    data[label] = value;
                }
            ...
            item.addEventListener("click", function (e) {                
                ...
                if (onClick === undefined) {
                    if (item === "add") {
                        ...
                        that.obj.group(firstNodeId, secondNodeId);
                    }
...
OrgChart.toolbarUI.prototype.init = function (obj, toolbar) {
    ...
    if (toolbar.fit){
        this.div.innerHTML += '<svg data-tlbr="fit" style="margin-bottom:7px;box-shadow: 0px 1px 4px rgba(0,0,0,0.3); border: 1px solid #cacaca;background-color: #f9f9f9;display: block;cursor: pointer;" width="32px" height="32px"><path stroke-width="3" fill="none" stroke="#757575" d="M4,11 L4,4 L11,4"></path><path stroke-width="3" fill="none" stroke="#757575" d="M28,11 L28,4 L21,4"></path><path stroke-width="3" fill="none" stroke="#757575" d="M28,21 L28,28 L21,28"></path><path stroke-width="3" fill="none" stroke="#757575" d="M4,21 L4,28 L11,28"></path><circle cx="16" cy="16" r="5" fill="#757575"></circle></svg>';
    }
    if (toolbar.zoom){
    ...
    if (toolbar.layout){
        this.div.innerHTML += '<svg data-tlbr="layout" style="box-shadow: 0px 1px 4px rgba(0,0,0,0.3); border: 1px solid #cacaca;background-color: #f9f9f9;display: block;cursor: pointer;" width="32px" height="32px"><path stroke-width="3" fill="none" stroke="#757575" d="M8,24 L16,14 L24,24"></path><path stroke-width="3" fill="none" stroke="#757575" d="M8,16 L16,8 L24,16"></path></svg>';
...
OrgChart.prototype._export = function (options, callback) {
    ...
    this._draw(false, OrgChart.action.exporting, params, function (content) {
        ...
        if (options.ext == "svg") {
            if (!callback) {
                var result = OrgChart.events.publish('exportstart', [that, options, el.innerHTML]);
                if (result === false) {
                    return false;
                }
                OrgChart._downloadFile(options.mime, el.innerHTML, options.filename);
                result = OrgChart.events.publish('exportend', [that, options, el.innerHTML]);
                if (result === false) {
                    return false;
                }
            }
                ...
                that._pages(options, el.querySelector('svg'), function(pages){
                    ...
                    if (!callback) {
                        ...
                        OrgChart._ajax(that.config.exportUrl + "/v3", "POST", req, "arraybuffer", function (response) {
                            ...
                            OrgChart._downloadFile(options.mime, response, options.filename);
                        });
                    }
                    else {
...
OrgChart.prototype.exportCSV = function (filename) {
    ...
    var result = OrgChart.events.publish('exportstart', [this, "csv", filename]);

    if (result === false) {
        return false;
    }

    var csv = OrgChart._json2csv(this.config.nodes);

    var result = OrgChart.events.publish('exportend', [this, "csv", filename, csv]);

    if (result === false) {
        return false;
    }

    OrgChart._downloadFile('text/csv;charset=utf-8;', csv, filename);
};
...
OrgChart.prototype.exportXML = function (filename) {
    ...
    var result = OrgChart.events.publish('exportstart', [this, "xml", filename]);

    if (result === false) {
        return false;
    }

    var xml = OrgChart._json2xml(this.config.nodes);

    var result = OrgChart.events.publish('exportend', [this, "xml", filename, xml]);

    if (result === false) {
        return false;
    }

    OrgChart._downloadFile('application/xml', xml, filename);
};
...
OrgChart.prototype._pages = function (opt, svg, callback) {
    ...
    else if ((opt.format == 'A4' && opt.scale == 100) 
        || (opt.format == 'A3' && opt.scale == 100)
        || (opt.format == 'A2' && opt.scale == 100)
        || (opt.format == 'A1' && opt.scale == 100)) {
        callback(this._pagesA100(opt, svg));
    }
...
OrgChart.prototype._pagesA100 = function (opt, svg) {
    var width = svg.getAttribute('width');
    var height = svg.getAttribute('height');
    ...
    while (temp_vx < vw) {
        while (temp_vy < vh) {
            var viewbox = [temp_vx, temp_vy, vw, vh];
            viewbox = viewbox.join();
...
OrgChart.prototype._nodeMouseDownHandler = function (sender, mde, normalizedEvents) {    
    ...
    if (bgnode.isChildOfGroup) {
        var parentGroupNodeElement = this.getNodeElement(bgnode._groupParentNodeId);
        ...
        if (dragStart) {            
            ...
            if (lastHoveredNodeId != null) {
                ...
                if (lastHoveredBGNode.isChildOfGroup) {
                    ...
                    lastHoveredNode.style.opacity = 1;
                }
                else if (!lastHoveredBGNode.isGroup) {
                    lastHoveredNode.style.opacity = 1;
                }
            }
    ...
    var leaveHandler = function (e) {
        ...
        if ((bgnode.id == lastHoveredNodeId) || (bgnode.isChildOfGroup && bgnode._groupParentNodeId == lastHoveredNodeId) || (lastHoveredNodeId == null)) {
            svg.removeChild(dragNodeElement);
            that._gragStartedId = null;
            return;
        }

        var targetBGNode = that.getBGNode(lastHoveredNodeId);
        if (that.config.dragDropMenu == null || (bgnode.isGroup && !targetBGNode.isGroup)) {            
            if (that._canUpdateLink(bgnode.id, lastHoveredNodeId)) {                
                that.linkNode(bgnode.id, lastHoveredNodeId);         
            }
            else {
                svg.removeChild(dragNodeElement);
            }
            ...
            if (that._canUpdateLink(bgnode.id, lastHoveredNodeId)) {
                ...
                var menu = null;
                if (Array.isArray(bgnode.tags)) {
                    for (var i = 0; i < bgnode.tags.length; i++) {
                        var tag = bgnode.tags[i];
                ...
                that.dragDropMenuUI.show(client.x, client.y, bgnode.id, lastHoveredNodeId, menu);
            }
            else {
                svg.removeChild(dragNodeElement);
            }
...
OrgChart.prototype._nodeClickHandler = function (id, e) {
    var result = OrgChart.events.publish('click', [this, this.get(id)]);
    if (result === false) {
        return false;
    }
...
OrgChart.prototype._nodeMenuClickHandler = function (id, el, e) {
    ...
    var position = OrgChart._menuPosition(el, e, this.getSvg());
    ...
    this.nodeMenuUI.show(position.x, position.y, id, null, menu);
};
...
OrgChart.prototype._lonelyButtonHandler = function () {
    var node = new OrgChart.node(OrgChart._randomId(), null, null, [], this.config.template);
    this.addNode(node);
    this.center(node.id);
};
...
OrgChart.node = function (id, pid, spids, tags, templateName) {
    this.templateName = templateName;    
    this.id = id;
    this.pid = pid;
    this.spids = Array.isArray(spids) ? spids : null;
    this.x = null;
    this.y = null;
    this.level = null;
...
OrgChart.server.prototype.find = function (search) {
    ...
    for (var i = 0; i < found.length; i++) {
        ...
        resultArray.push({
            id: found[i].id,
            node: new OrgChart.node(found[i], null, null, [], templateName),
            textId: textId,
...
OrgChart.server._createNodes = function (config, action, actionParams, oldNodes) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        var spids = Array.isArray(d.spids) ? d.spids : null;
        var node = new OrgChart.node(d.id, d.pid, spids, tags, templateName);
        node.isAssistant = (tags.indexOf("assistant") != -1);
        if (node.isAssistant && assistantPids.indexOf(node.pid) == -1) {
            assistantPids.push(node.pid);
        }
        if (group != null) {
            if (!nodes[group.name]) {
                nodeIds.push(group.name);
                var groupNode = new OrgChart.node(group.name, d.pid, null, [group.name], group.tag.template);
                groupNode.isGroup = true;
                if (config.orderBy != null) {
                    groupNode.order = d[config.orderBy];
                }
    ...
    for (var i = 0; i < assistantPids.length; i++) {
        ...
        var slnode = new OrgChart.node(pnode.id + "_split_0", pnode.id, null, [], "split"); //last split node    
        ...
        for (var j = pnode.children.length - 1; j >= 0; j--) {
            ...
            if (cnode.isAssistant) {
                ...
                cnode.pid = slnode.id;
                cnode.parent = slnode;
                slnode.children.push(cnode);
                slnode.childrenIds.push(cnode.id);
                pnode.children.splice(j, 1);
                //pnode.childrenIds.splice(j, 1);                
            }
        }
        ...
        if (anodeIds.length % 2) { //odd
            ...
            var tnode = new OrgChart.node(pnode.id + "_transperant", lanode.pid, null, [], tName); //transperant node 
        ...
        for (var j = anodeIds.length - 1; j >= 0; j--) {
            if (j % 2 && j != anodeIds.length - 1) {//odd
                var snode =  new OrgChart.node(pnode.id + "_split_" + autoIncrement, null, null, [], "split"); //split node   
                ...
                if (pnode.children[j] == node) {
                    ...
                    if (node.templateName == "split" || pnode.templateName == "split") {
                        ...
                        if (psnode) {
                            ...
                            for (var o = 0; o < psnode.childrenIds.length; o++) {
                ...
                if (!OrgChart.server._mixedLayoutAllNodes(pnode)){
                 ...
                 if (cnode.childrenIds.length == 0 && !cnode.isAssistant && cnode.templateName != "split" && (cnode.parent && cnode.parent.templateName != "split" || cnode.parent.isAssistant)) { //#106
                     ...
                     if (config.layout == OrgChart.treeLeftOffset || config.layout == OrgChart.treeRightOffset) {                         
                         ...
                         var tnode = new OrgChart.node(cnode.id + "_transperant", null, null, [], tName); //transperant node 
            ...
            switch (orientation) {
                ...
                case OrgChart.orientation.bottom_left:
                    ...
                    for (var i = 0; i < node.bgnodes.length; i++) {
                        ...
                        h += node.bgnodes[i].h + t.groupNodesSeparation;
                    }
                    h -= t.groupNodesSeparation;
                ...
                case OrgChart.orientation.left_top:
                    ...
                    for (var i = 0; i < node.bgnodes.length; i++) {
                        ...
                        w += node.bgnodes[i].w + t.groupNodesSeparation;
                    }
                    w -= t.groupNodesSeparation;
...
OrgChart.server.getAllFields = function (config) {
    ...
    for (var i in config.nodeParams) {
...
OrgChart.server._getResponse = function (width, height, oldVisibleNodes, config, maxX, maxY, minX, minY, viewBox, firstRoot, action, actionParams, nodes, groupChildren, oldNodes) {
    ...
    var scale = OrgChart.getScale(viewBox, width, height, config.scaleInitial, inX, inY);
...
OrgChart.ui = {
    ...
    node: function (bgnode, data, animations, config, x, y, nodeBinding, action, scale, sender) {
        ...
        if (bgnode.isGroup) {
            ...
            if (groupName) {                
                groupName = OrgChart._wrapText(groupName, t.groupName);
                var fieldHtml = t.groupName.replace("{val}", groupName);
                fieldHtml = fieldHtml
                ...
                if (data) {
                    ...
                    if (replacement != undefined && replacement != null && t[f] != undefined) {
                        ...
                        if (!OrgChart._lblIsImg(config, f) && typeof (replacement) == "string") {
                            ...
                            replacement = OrgChart._wrapText(replacement, t[f]);
    ...
    secondLinks: function (nodes, node, config, boundary, scale) {
        if (node.spids == null) {
            return "";
        }
        ...
        for (var i = 0; i < node.spids.length; i++) {
            var path = [];
            var spid = node.spids[i];
            var parentNode = null;
            var align = "left";
            ...
            switch (config.orientation) {
                ...
                case OrgChart.orientation.right_top:
                    align = "top";
                    if (nodes[spid].y > node.y) {
                        align = "bottom";
                    }
                    break;
            }

            var t = OrgChart.t(node.templateName, scale);
            var separation = config.levelSeparation;
            if (node.parent && node.parent.layout == OrgChart.mixed || node.parent.layout == OrgChart.tree) {
                separation = config.mixedHierarchyNodesSeparation;
            }  
            var point = {
                p: node.x + node.w / 2 + t.expandCollapseSize,
                q: node.y,
                r: node.x + node.w / 2 + t.expandCollapseSize,
            ...
            switch (config.orientation) {
                ...
                case OrgChart.orientation.top_left:
                    ...
                    path.push([point.r, node.y - separation / 3]);
                    break;
                case OrgChart.orientation.bottom:
                case OrgChart.orientation.bottom_left:
                    point.q = node.y + node.h;
                    point.s = boundary.maxY - separation;
                    path.push([point.p, point.q]);
                    path.push([point.r, node.y + node.h + separation / 3]);                    
                    break;
                case OrgChart.orientation.left:
                case OrgChart.orientation.left_top:
                    point.p = node.x;
                    point.q = node.y + node.h / 2 + t.expandCollapseSize;
                    point.r = boundary.minX - separation;
                    point.s = node.y + node.h / 2 + t.expandCollapseSize;
                    path.push([point.p, point.q]);
                    path.push([node.x - separation / 3, point.q]);
                    break;
                case OrgChart.orientation.right:
                case OrgChart.orientation.right_top:
                    point.p = node.x + node.w;
                    point.q = node.y + node.h / 2 + t.expandCollapseSize;
                    point.r = boundary.maxX + separation;
                    point.s = node.y + node.h / 2 + t.expandCollapseSize;
                    path.push([point.p, point.q]);
                    path.push([node.x + node.w + separation / 3, point.q]);
                    break;
            }
            ...
            while (parentNode == null) {
                ...
                else if (OrgChart._intersects(pn, point, config)) {
                    point = OrgChart._addPoint(pn, path, config, point, align);
                    hasIntersect = true;
                }
                ...
                if (pn.id != spid) {
                    while (ln) {
                        ...
                        if (OrgChart._intersects(ln, point, config)) {
                            point = OrgChart._addPoint(ln, path, config, point, align);
                            hasIntersect = true;
                        }
                        ln = ln.leftNeighbor;
                    }
                    ...
                    while (rn) {
                        ...
                        if (OrgChart._intersects(rn, point, config)) {
                            point = OrgChart._addPoint(rn, path, config, point, align);
                            hasIntersect = true;
                        }
                        rn = rn.rightNeighbor;
                    }
                ...
                if (!hasIntersect) {
                    ...
                    var y = 0;
                    if (pn.parent) {
                        separation = config.levelSeparation;
            ...
            html += OrgChart.secondLinkOpenTag
                .replace("{id}", node.id)
                .replace("{child-id}", spid)
                + t.secondLink.replace("{d}", path);
...
OrgChart.getScale = function (viewBox, w, h, initial, x, y) {
    if (!viewBox && initial === OrgChart.match.boundary) {
        var scaleX = (w) / x;
        var scaleY = (h) / y;
        return scaleX > scaleY ? scaleY : scaleX;
    }
    if (!viewBox && initial === OrgChart.match.width) {
        return (w) / x;
    }
    if (!viewBox && initial === OrgChart.match.height) {
        return (h) / y;
    }
    else if (!viewBox) {
        return initial;
...
OrgChart._browser = function () {
    ...
    return { msie: (navigator.userAgent.indexOf("Trident") != -1), webkit: browser.browser == "webkit", mozilla: browser.browser == "mozilla", opera: browser.browser == "opera" };
};


OrgChart._menuPosition = function (el, e, svg) {
    var x = e.offsetX;
    var y = e.offsetY;
    var senderb = el.getBoundingClientRect();
    var svgb = svg.getBoundingClientRect();
    x = senderb.left - svgb.left;
    y = senderb.top - svgb.top;
...
OrgChart._wrapText = function (text, field) {
    ...
    var testDiv = document.getElementById("bg-tests");
...
OrgChart._downloadFile = function (type, content, filename) {
...
OrgChart.xScrollUI.prototype.addListener = function (svg) {
    var that = this;
    if (this.config.mouseScrool != OrgChart.action.xScroll) {
        return;
    }
...
OrgChart.yScrollUI.prototype.addListener = function (svg) {
    var that = this;
    if (this.config.mouseScrool != OrgChart.action.yScroll) {
        return;
    }
    if (!this.bar) {//disabled
        return;
...
OrgChart.yScrollUI.prototype.setPosition = function () {
    ...
    var p = this.requestParams();
    var innerHeight = p.boundary.maxY * p.scale;
    var innerWidth = p.boundary.maxX * p.scale;
...
OrgChart.prototype._clink = function(args){
    var html = '';
    for (var i = 0; i < this.config.clinks.length; i++) {
        var clink = this.config.clinks[i];

        var fromnode = args.res.visibleNodes[clink.from];
        var tonode = args.res.visibleNodes[clink.to];
        ...
        var c_fromnode_x = (fromnode.x + fromnode.w / 2);
        var c_tonode_x = (tonode.x + tonode.w / 2);
        var c_fromnode_y = (fromnode.y + fromnode.h / 2);
        var c_tonode_y = (tonode.y + tonode.h / 2);
        ...
        switch (this.config.orientation) {
            ...
            case OrgChart.orientation.top_left:
                ...
                if (fromnode.y == tonode.y){
                    ...
                    a.y = fromnode.y;
                    b.y = tonode.y + tonode.h;
                }
                else{
                    a.y = fromnode.y + fromnode.h;
                    b.y = tonode.y;
                }
                break;
            case OrgChart.orientation.bottom:
            ...
            case OrgChart.orientation.left_top:
                ...
                if (fromnode.x == tonode.x){
                    ...
                    a.x = fromnode.x;
                    b.x = tonode.x + tonode.w;
                }
                else{
                    a.x = fromnode.x + fromnode.w;
                    b.x = tonode.x;
                }
                break;
            case OrgChart.orientation.right:
